...
*** =instructions
- Take a peek at the code written on the right in script.R. When you opened this exercise, the code gave your computer the instructions to (1) load a dataset and (2) produce the graphic next to it.
...
# Draw a graph of student shoesize and height using plot()
with(students2014, plot(x = kenka, y = pituus, col = sukup, pch = 20, main = "Scatter plot of student shoesize and height \n(color indicates sex)"))
...
*** =instructions
- Execute the example codes where the function `head()` is used to explore the first couple observations of the `students2014` dataset.
...
- Create the object `my_boolean` by executing the line. Note that with boolean values, no quotation marks are needed.
...
You can create vectors by combining values of the same data type, using the function `c()`. In this exercise you will create your own vectors.  
