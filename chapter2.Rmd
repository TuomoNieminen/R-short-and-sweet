...
description : Logic is the beginning of wisdom, not the end
...
- Draw a (scatter) plot of student shoesize versus height, with shoesize on the x axis and height on the y axis. 
...
Logical conditions are generally important in programming tasks. In R, logical comparison creates logical vectors. The logical comparison operators in R are:  
...
*** =instructions
- Loop though the character values using the example code. Here in DataCamp, executing a command over multiple lines is done by selecting (painting) all the lines with a mouse first and then hitting `Ctrl+Enter` normally. Alternatively you could also click submit to execute the whole script.R
...
There are countless of ready-made functions for you in R. At this point of the course, we have used many functions, for example `str()`, `summary()`, `head()`, `plot()` and `mean()`. But it is possible to create your own functions too. Now we will look at the definition of a simple function and learn how to use it (*call* it).

Firts a little recap on using functions:
